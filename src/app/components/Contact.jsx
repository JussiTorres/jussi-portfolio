...
export default function Contact() {
  ...
  // Unificamos TODOS los enlaces para que reaccionen en ROJO (Terminal Red Theme)
  const socialLinks = [
    {
      name: "GitHub",
  ...
  return (
    <footer id="contact" className="relative bg-neutral-950 pt-32 pb-12 overflow-hidden border-t border-white/5">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          {/* Typography match: font-black uppercase tracking-tighter */}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-20">
          {socialLinks.map((link, index) => (
            <motion.a
              ...
              className="flex flex-col items-center justify-center p-8 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-sm transition-all duration-300 group hover:border-red-600 hover:bg-red-900/10 cursor-pointer"
            >
              <div className="mb-4 text-gray-400 group-hover:text-red-500 transition-colors transform group-hover:scale-110 duration-300">
                {link.icon}
              </div>
              <span className="text-sm font-bold text-gray-500 group-hover:text-white uppercase tracking-wider">
                {link.name}
              </span>
            </motion.a>
          ))}
